...
## 🧭 Instalación en tus agentes (un comando)

OurBook detecta qué agentes MCP tienes instalados y te deja elegir dónde registrar el servidor:

```bash
npx ourbook setup            # detecta agentes y te deja elegir (interactivo)
npx ourbook setup --yes      # instala en todos los agentes detectados
npx ourbook setup --agents claude-desktop,cursor --dry-run   # solo previsualiza
npx ourbook agents           # solo muestra lo detectado
npx ourbook uninstall        # retira OurBook de un agente
```

**Agentes soportados:** Claude Desktop · Claude Code (CLI) · Cursor · OpenCode · Windsurf · VS Code (Copilot) · Zed · Gemini CLI

El asistente escribe el formato nativo de cada agente (`mcpServers` / `servers` / `mcp` / `context_servers` / `mcp_servers`), **hace backup** de la configuración antes de tocarla, registra `ourbook` con un comando `node` absoluto y nunca toca tus otros servidores. Tras instalar, reinicia el agente (o `/mcp` en Claude Code).
